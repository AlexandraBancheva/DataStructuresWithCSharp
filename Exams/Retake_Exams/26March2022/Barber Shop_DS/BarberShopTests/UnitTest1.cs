...
public class Test_01
{
    ...
    // My tests
    [Test]
    public void TestAssignClientThrowsException()
    {
        this.barberShop.AddBarber(b1);
        this.barberShop.AddClient(c1);

        Assert.Throws<ArgumentException>(() => barberShop.AssignClient(b3, c4));
    }

    [Test]
    public void TestIfAssignClientWorksCorrectly()
    {
        this.barberShop.AddBarber(b1);
        this.barberShop.AddClient(c1);
        ...
        var actual = 1;
        var expected = b1.Clients.Count;
        Assert.AreEqual(expected, actual);
    }

    [Test]
    public void TestIfDeleteAllClientsFromBarberThrowsArgumentException() 
    {
        Assert.Throws<ArgumentException>(() => this.barberShop.DeleteAllClientsFrom(b1));
    }
